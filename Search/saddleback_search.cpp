...
#include <iostream>     /// for io operations, and pair
 ...
 * return An pair of with row and column populated within it, if the
 * element is present.
 * return An pair with (0, 0), if the element is not present.
 */
pair<uint32_t, uint32_t> saddleback(vector<vector<int32_t>> matrix,
                               int32_t element) {
    ...
    while (left_index < matrix.size()) {  // Exit once the value of indexes get out of range.
        if (element ==
            ...
            return make_pair(left_index+1, right_index+1);
        } else if (element >
                   matrix[left_index]
...
static void test() {
    vector<vector<int32_t>> matrix = {{1, 10, 100, 1000, 10000},
                                            {2, 20, 200, 2000, 20000},
                                            {3, 30, 300, 3000, 30000},
                                            {4, 40, 400, 4000, 40000},
                                            {5, 50, 500, 5000, 50000}};

    pair<uint32_t, uint32_t> not_found = make_pair(0, 0);
    pair<uint32_t, uint32_t> test_answer;
    // Test 1
    pair<uint32_t, uint32_t> answer1 = search::saddleback::saddleback(matrix, 123);
