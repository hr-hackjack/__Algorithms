...
template<typename T>
vector<T> computeLogs(const vector<T>& A) {
    int n = A.size();
    vector<T> logs(n);
    logs[1] = 0;
    for (int i = 2 ; i < n ; i++) {
 ...
 */
template<typename T>
vector<vector<T> > buildTable(const vector<T>& A, const vector<T>& logs) {
    int n = A.size();
    vector<vector<T> > table(20, vector<T>(n+5, 0));
    int curLen = 0;
 ...
 */
template<typename T>
int getMinimum(int beg, int end, const vector<T>& logs, const vector<vector<T> >& table) {
 ...
 */
int main() {
    vector<int> A{1, 2, 0, 3, 9};
    vector<int> logs = range_queries::sparse_table::computeLogs(A);
    vector<vector<int> >  table = range_queries::sparse_table::buildTable(A, logs);
