...
class geometric_distribution {
 ...
 public:
    ...
    float probability_density(const uint32_t& k) const {
        return pow((1.0f - p), static_cast<float>(k - 1)) * p;
    }
    ...
    float cumulative_distribution(const uint32_t& k) const {
        return 1.0f - pow((1.0f - p), static_cast<float>(k));
    }
    ...
    float inverse_cumulative_distribution(const float& cdf) const {
        return log(1.0f - cdf) / log(1.0f - p);
    }
    ...
    float range_tries(const uint32_t& min_tries = 1,
                      const uint32_t& max_tries =
                          numeric_limits<uint32_t>::max()) const {
        float cdf_lower = cumulative_distribution(min_tries - 1);
        float cdf_upper = max_tries == numeric_limits<uint32_t>::max()
                              ? 1.0f
...
void sample_test(
    ...
    cout << "This value should be near " << dist.expected_value() << ": "
              << mean << endl;
    cout << "This value should be near " << dist.variance() << ": " << var
              << endl;
}
...
static void test() {
    ...
    cout << "Starting tests for p = 0.3..." << endl;
    assert(abs(dist.expected_value() - 3.33333333f) < threshold);
    assert(abs(dist.variance() - 7.77777777f) < threshold);
    assert(abs(dist.standard_deviation() - 2.788866755) < threshold);
    assert(abs(dist.probability_density(5) - 0.07203) < threshold);
    assert(abs(dist.cumulative_distribution(6) - 0.882351) < threshold);
    assert(abs(dist.inverse_cumulative_distribution(
                        dist.cumulative_distribution(8)) -
                    8) < threshold);
    assert(abs(dist.range_tries() - 1.0f) < threshold);
    assert(abs(dist.range_tries(3) - 0.49f) < threshold);
    assert(abs(dist.range_tries(5, 11) - 0.2203267f) < threshold);
    cout << "All tests passed" << endl;
    sample_test(dist);

    dist = probability::geometric_dist::geometric_distribution(0.5f);

    cout << "Starting tests for p = 0.5..." << endl;
    assert(abs(dist.expected_value() - 2.0f) < threshold);
    assert(abs(dist.variance() - 2.0f) < threshold);
    assert(abs(dist.standard_deviation() - 1.4142135f) < threshold);
    assert(abs(dist.probability_density(5) - 0.03125) < threshold);
    assert(abs(dist.cumulative_distribution(6) - 0.984375) < threshold);
    assert(abs(dist.inverse_cumulative_distribution(
                        dist.cumulative_distribution(8)) -
                    8) < threshold);
    assert(abs(dist.range_tries() - 1.0f) < threshold);
    assert(abs(dist.range_tries(3) - 0.25f) < threshold);
    assert(abs(dist.range_tries(5, 11) - 0.062011f) < threshold);
    cout << "All tests passed" << endl;
    sample_test(dist);

    dist = probability::geometric_dist::geometric_distribution(0.8f);

    cout << "Starting tests for p = 0.8..." << endl;
    assert(abs(dist.expected_value() - 1.25f) < threshold);
    assert(abs(dist.variance() - 0.3125f) < threshold);
    assert(abs(dist.standard_deviation() - 0.559016f) < threshold);
    assert(abs(dist.probability_density(5) - 0.00128) < threshold);
    assert(abs(dist.cumulative_distribution(6) - 0.999936) < threshold);
    assert(abs(dist.inverse_cumulative_distribution(
                        dist.cumulative_distribution(8)) -
                    8) < threshold);
    assert(abs(dist.range_tries() - 1.0f) < threshold);
    assert(abs(dist.range_tries(3) - 0.04f) < threshold);
    assert(abs(dist.range_tries(5, 11) - 0.00159997f) < threshold);
