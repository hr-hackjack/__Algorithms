/**
 * The [Z function](https://cp-algorithms.com/string/z-function.html) for
...
vector<uint64_t> Z_function(const string &pattern) {
    uint64_t pattern_length = pattern.size();
    vector<uint64_t> z(pattern_length, 0);
 ...
 */
vector<uint64_t> find_pat_in_text(const string &pattern,
                                       const string &text) {
    uint64_t text_length = text.size(), pattern_length = pattern.size();
    vector<uint64_t> z = Z_function(pattern + '#' + text);
    vector<uint64_t> matching_indexes;
...
static void test() {
    // usual case
    string text1 = "alskfjaldsabc1abc1abcbksbcdnsdabcabc";
    string pattern1 = "abc";

    // matching_indexes1 gets the indexes where pattern1 exists in text1
    vector<uint64_t> matching_indexes1 = find_pat_in_text(pattern1, text1);
    assert((matching_indexes1 == vector<uint64_t>{10, 14, 18, 30, 33}));

    // corner case
    string text2 = "greengrass";
    string pattern2 = "abc";

    // matching_indexes2 gets the indexes where pattern2 exists in text2
    vector<uint64_t> matching_indexes2 = find_pat_in_text(pattern2, text2);
    assert((matching_indexes2 == vector<uint64_t>{}));
}
